...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self._h_weights = np.random.rand(n_features, self.n_hidden)
        self._h_bias = np.random.rand(1, self.n_hidden)
        self._o_weights = np.random.rand(self.n_hidden, n_outputs)
        self._o_bias = np.random.rand(1, n_outputs)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_iterations):
            ...
            # Forward
            h_val = np.dot(self._X, self._h_weights) + self._h_bias
            h_out = self.hidden_activation(h_val)

            o_val = np.dot(h_out, self._o_weights) + self._o_bias
            y_pred = self._output_activation(o_val)
            ...
            delta_o = self._output_activation(o_val, derivative=True) * error
            grad_o = np.dot(h_out.T, delta_o)

            delta_h = self.hidden_activation(h_val, derivative=True) * np.dot(delta_o, self._o_weights.T)
            grad_h = np.dot(self._X.T, delta_h)

            self._o_weights -= self.learning_rate * grad_o
            self._h_weights -= self.learning_rate * grad_h

            self._o_bias -= self.learning_rate * np.sum(delta_o, axis=0)
            self._h_bias -= self.learning_rate * np.sum(delta_h, axis=0)
    ...
    def predict(self, X):
        ...
        """
        h_in = np.dot(X, self._h_weights) + self._h_bias
        h_out = self.hidden_activation(h_in)

        o_in = np.dot(h_out, self._o_weights) + self._o_bias
        y_pred = self._output_activation(o_in)
