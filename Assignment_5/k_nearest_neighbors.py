...
class KNearestNeighbors:
    ...
    def predict(self, X):
        ...
        for test_item in X:
            distances = []
            for train_item, label in zip(self._X, self._y):
                distance = self._distance(train_item, test_item)
                distances.append((distance, label))
            nearest = sorted(distances, key=lambda x:x[0])[:self.n_neighbors]

            weights = []
            if self.weights == 'uniform':
                weights = [(1, label) for _, label in nearest]
            else:
                if any(distance == 0 for distance, _ in nearest):
                    weights = [(1, label) for distance, label in nearest if distance == 0]
                else:
                    weights = [(1 / distance, label) for distance, label in nearest]

            w = {}
            for distance, label in weights:
                if label in w:
                    w[label] += distance
                else:
                    w[label] = distance
            predictions.append(max(w, key=w.get))   
        return predictions
